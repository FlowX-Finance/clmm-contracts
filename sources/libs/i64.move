// Copied from: https://github.com/CetusProtocol/integer-mate/blob/main/sui/sources/i64.move
module flowx_clmm::i64 {
    ...
    public fun from(v: u64): I64 {
        assert!(v <= MAX_AS_U64, E_OVERFLOW);
        I64 {
            bits: v
        }
    }
    ...
    public fun neg_from(v: u64): I64 {
        assert!(v <= MIN_AS_U64, E_OVERFLOW);
        if (v == 0) {
            I64 {
                bits: v
            }
    ...
    public fun abs(v: I64): I64 {
        if (sign(v) == 0) {
            ...
            assert!(v.bits > MIN_AS_U64, E_OVERFLOW);
            I64 {
